...
public class SetCover {
    ...
    public static List<int[]> chooseSets(List<int[]> sets, int[] universe) {
        ...
        while (!universeSet.isEmpty()) {
            int smallestCountForMatchingElements = 0;
            int[] chosenSet = sets.get(0);

            for (int[] set : sets) {
                int matchingElementsInCurrentSet = 0;

                for (int element : set) {
                    if (universeSet.contains(element)) matchingElementsInCurrentSet++;
                }

                if (smallestCountForMatchingElements < matchingElementsInCurrentSet) {
                    smallestCountForMatchingElements = matchingElementsInCurrentSet;
                    chosenSet = set;
                }
            }
